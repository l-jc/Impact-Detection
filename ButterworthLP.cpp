...
void ButterworthLP::vecfilter(const std::vector<double> &input, std::vector<double> &output, const size_t size, const double initialConditionValue, const bool forwardBackward)
{
    // Save all current SOS states before offline filtering
    std::vector<double> zSaved(_numSOS * 2);
    for (size_t i = 0; i < _numSOS; ++i)
